...
from drunkard_server import DrunkardServer

# Our new main class doesn't collect data, but instead simply starts the server, which waits for input via HTTP.

# The HTTP server is powered by our handler, which is the drunkard server we've already written.
...
server = HTTPServer(('0.0.0.0', 8888), DrunkardServer)
