...
async function main() {
    ...
    const width = await userInput.getInput('Enter width of space: ');
    const height = await userInput.getInput('Enter height of space: ');
    // Pre-construct body here for neatness, but could place in HTTP call directly.
    // This structure matches the format expected by the server, e.g.: '{"width":4, "height":4}'; we are adhering to the 'contract'.
    ...
    userInput.endInput();
    // We now have all the information we need to make a request to move the simulated drunkard, on the server, to the coordinates we have provided.
    ...
    // Our utility class extracts the body of the response for us (the drunkard's path), so we can just print directly.
